...
- **注入Mod(3dmigoto)**
  1. 请点击设置选项卡-Mod(3dmigoto)工具-一键注入按钮。程序将启动注入程序及原神主程序。如果在启动后看见程序上下边栏出现绿色的字母/数字，则说明注入成功。
  2. 请点击设置选项卡-Mod(3dmigoto)工具-打开mod文件夹按钮，把解压好的mod放入文件夹中，重启游戏即可。
...
- **GM工具**
  ...
  3. 当游戏内/控制台提示：`你没有权限执行此命令` 时，请在一键端设置选项卡-解锁账号权限输入你要解锁的UID，账号即开放所有权限。
